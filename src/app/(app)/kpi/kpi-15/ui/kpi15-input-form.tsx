...
import {
  KPI15_DATABASE_OPTIONS,
  type Kpi15Input,
} from "../../schemas/kpi-schemas";

interface Kpi15InputFormProps {
  form: UseFormReturn<Kpi15Input>;
  onSubmit: (data: Kpi15Input) => void;
  onReset: () => void;
}

export function Kpi15InputForm({
  form,
  onSubmit,
  onReset,
}: Kpi15InputFormProps) {
  ...
  const selectedLevel = watch("databaseLevel");
        ...
        <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
          <div className="rounded-md border border-orange-200 bg-orange-50/50 px-4 py-3 dark:border-orange-900 dark:bg-orange-950/20">
            <p className="font-medium text-orange-700 text-xs uppercase tracking-wide dark:text-orange-400">
              Subset 15.1 — Customer Database Management
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-start gap-3">
              ...
              <Label className="font-medium text-sm leading-snug">
                Select the level that best describes the utility&apos;s customer
                database management:
              </Label>
            </div>
            ...
            <div className="ml-9">
              <RadioGroup
                ...
                onValueChange={(val) =>
                  setValue("databaseLevel", val as "A" | "B" | "C" | "D" | "E")
                }
                value={selectedLevel}
              >
                {KPI15_DATABASE_OPTIONS.map((option) => (
                  <div
              ...
              {errors.databaseLevel && (
                <p className="mt-2 text-destructive text-xs">
                  {errors.databaseLevel.message}
                </p>
              )}
            </div>
          </div>
